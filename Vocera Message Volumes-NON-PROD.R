...
if (filesource == FALSE){
  ...
  write.csv(ds_TextConversationRcpDistLists <- sqlQuery(dbhandle, "SELECT * FROM [WICMASTER].[dbo].[TextConversationRcpDistLists] where InTime > '2017-07-10'"), 'dataset/ds_TextConversationRcpDistLists.csv')
  write.csv(ds_TextConversationRcpUsers <- sqlQuery(dbhandle, "SELECT * FROM [WICMASTER].[dbo].[TextConversationRcpUsers] where InTime > '2017-07-10'"), 'dataset/ds_TextConversationRcpUsers.csv')
  write.csv(ds_TextConversationParticipants <- sqlQuery(dbhandle, "SELECT * FROM [WICMASTER].[dbo].[TextConversationParticipants]where InTime > '2017-07-10'"), 'dataset/ds_TextConversationParticipants.csv')
  write.csv(ds_TextConversations <- sqlQuery(dbhandle, "SELECT * FROM [WICMASTER].[dbo].[TextConversations] where Created > '2017-07-10'"), 'dataset/ds_TextConversations.csv')
  odbcCloseAll()
} else{
...
if (titlevisualization == FALSE){
  ...
  sankeyNetwork(Links = sankey$links, Nodes = sankey$nodes, Source = "source",
                Target = "target", Value = "value", NodeID = "name",
                units = "\n messages sent", fontSize = 20, nodeWidth = 40, sinksRight = TRUE, iterations = 400, nodePadding = 40)
}
